...
> [!TIP]
> **New to Bitfields?** See our [Ecosystem Comparison](comparisons.md) to understand how `bitstruct` differs from `modular-bitfield`, `packed_struct`, and standard Rust enums.
...
## ⚖️ The Deep Dive: `std` vs. `bitstruct`

Chosen between standard Rust types and `bitstruct` depends on whether you are optimizing for **Developer Velocity** (std) or **Mechanical Sympathy** (bitstruct).
...
The `bitstruct` crate provides four specialized tools. Choosing the right one determines your memory density and instruction efficiency:
...
`bitstruct` is engineered for high-performance systems where data must move seamlessly between the CPU, the network, and other languages.
...
Because `bitstruct` uses **LSB-first** mapping, your logical definitions perfectly match the physical bit-offsets used in hardware datasheets for little-endian architectures (x86_64, ARM64).
...
// 1 Billion records = 4.0GB with bitstruct vs 8.0GB+ with standard structs.
```
...
`bitstruct` provides four specialized macros, each targeting a specific layer of the storage-performance spectrum.
...
`bitstruct` is engineered for **Mechanical Sympathy**. While standard Rust types are optimized for simplicity, `bitstruct` allows you to trade a negligible amount of instruction latency for massive gains in memory density (e.g., 2x - 8x).
...
| :--- | :--- | :--- | :--- | :--- |
| **Execution Latency** | `bitenum!` | `u8` (3 bits) | **0.92x** (Strict) | **1.00x** (Type Safe) |
| **Execution Latency** | `byteval!` | `[u8; 3]` | **1.00x** (Equal) | **2.67x Higher** |
| **Execution Latency** | `bitstruct!` | `u16` | **1.36x** (Atomic) | **2.00x Higher** |
| **Execution Latency** | `bytestruct!` | `[u8; 2]` | **3.84x** (Optimized) | **3.20x Higher** |
